...
/**
 * This is an event listener for Spring Security events that will see those events and re-send them to the
 * {@link EventManager}.
 *
 * This relies on the Spring event handling mechanism, and specifically on the events that Spring Security emits.
 *
 * @author John Melton (jtmelton@gmail.com) http://www.jtmelton.com/
 */
@Named
public class AppSensorSecurityEventListener implements ApplicationListener<ApplicationEvent> {
  ...
  public void onApplicationEvent(ApplicationEvent applicationEvent) {
    ...
    if (applicationEvent instanceof AbstractAuthenticationFailureEvent) {
      ...
      this.ids.addEvent(new Event(user, detectionPoint, getDetectionSystem()));

    } else if (applicationEvent instanceof AuthenticationSuccessEvent
        || applicationEvent instanceof InteractiveAuthenticationSuccessEvent) {
  ...
  private IPAddress getUserIp(Authentication authentication) {
    ...
    // return this.locator.fromString("129.187.7.5");
    return this.locator.fromString(remoteAddress);
  }
  ...
  private DetectionSystem getDetectionSystem() {
    ...
    // TODO FIX here
    return new DetectionSystem(this.appSensorClient.getConfiguration().getServerConnection()
        .getClientApplicationIdentificationHeaderValue(), this.locator.fromString("10.40.144.201"));
  }

  private String getApplicationIp() {

    // FIXME get IP Adresse out of interface
    if (cachedIp != null) {
      return cachedIp;
    }
